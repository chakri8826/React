...
class App extends Component {
  ...
  onThumbnailClick = (thumbnailUrl) => {
    ...
    const matchedImage = imagesList.find(
      (img) => img.thumbnailUrl === thumbnailUrl
    );

    if (matchedImage.imageUrl === imageToMatch) {
      const newImage =
        imagesList[Math.floor(Math.random() * imagesList.length)];
      this.setState((prevState) => ({
        score: prevState.score + 1,
        imageToMatch: newImage.imageUrl,
      }));
    } else {
      this.endGame();
    }
  ...
  render() {
    const { score, time, isGameRunning, activeTabId, imageToMatch } =
      this.state;
      ...
      <div className="app-container">
        ...
        {isGameRunning ? (
          <div className="game-container">
            <img src={imageToMatch} alt="match" className="match-img" />
            <ul className="tabs-list">
              {tabsList.map((tab) => (
                <li key={tab.tabId}>
                  <button
                    className={activeTabId === tab.tabId ? "tab active" : "tab"}
                    onClick={() => this.onTabClick(tab.tabId)}
                  >
                    {tab.displayText}
                  </button>
                </li>
              ...
              {filteredImages.map((img) => (
                <li key={img.id}>
                  <button
                    ...
                    onClick={() => this.onThumbnailClick(img.thumbnailUrl)}
                  >
                    <img
          ...
          <div className="score-card"> 
            ...
            <button className="play-again-btn" onClick={this.onResetGame}>
              <img
